...
export default function EditExercisePage() {
  ...
  const [loading, setLoading] = useState(true); // Für das Laden der Daten
  const [saving, setSaving] = useState(false);  // Für das Speichern
  ...
  useEffect(() => {
    const fetchExercise = async () => {
      ...
      if (data && !error) {
        ...
        setMaxPlayers(data.max_players || 12);
  ...
  const handleUpdate = async () => {
    ...
    try {
      let finalImageUrl = imagePreview; // Standardmäßig das alte Bild behalten

      // Nur hochladen, wenn ein neues Bild gewählt wurde (processedImage existiert)
      if (processedImage) {
