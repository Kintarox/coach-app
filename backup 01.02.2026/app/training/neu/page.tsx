...
export default function NewTrainingSessionPage() {
  ...
  // PHASEN STATE (Jetzt mit 'coolDown')
  ...
  // --- DRAG & DROP LOGIK (Advanced) ---

  // Start: Aus dem Katalog (NEU)
  ...
  // Start: Aus einer Phase (VERSCHIEBEN)
  ...
  // Drop Handler (Unterscheidet zwischen NEU und VERSCHIEBEN)
    ...
    try {
      ...
      setPhases(prev => {
        ...
        if (item.type === 'MOVE' && item.sourcePhase !== undefined && item.sourceIndex !== undefined) {
          // Nur löschen, wenn Quelle != Ziel (oder um es ans Ende zu schieben)
    ...
    <div className="bg-[#F5F5F7] min-h-screen ml-64 p-6 pb-40 flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex gap-6 flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar pb-20">
            ...
            <PhaseDropZone 
                phaseKey="warmup" 
                title="Einstimmen / Aufwärmen" 
                meta="ca. 10-15 Min • Spielerisch" 
                color="orange" 
                number="1"
                exercises={phases.warmup} 
                onDrop={handleDrop} 
            ...
            <PhaseDropZone 
                phaseKey="main1" 
                title="Hauptteil I - Technik" 
                meta="ca. 15-20 Min • Intensiv" 
                color="blue" 
                number="2"
                exercises={phases.main1} 
                onDrop={handleDrop} 
                onDragOver={handleDragOver}
                onDragStartMove={handleDragStartMove}
                onRemove={removeExercise}
            ...
            <PhaseDropZone 
                phaseKey="main2" 
                title="Hauptteil II - Spielform" 
                meta="ca. 25-30 Min • Anwendung" 
                color="purple" 
                number="3"
                exercises={phases.main2} 
                onDrop={handleDrop} 
                onDragOver={handleDragOver}
                onDragStartMove={handleDragStartMove}
                onRemove={removeExercise}
            ...
            <PhaseDropZone 
                phaseKey="coolDown" 
                title="Ausklang" 
                meta="ca. 5-10 Min • Cool Down" 
                color="green" 
                number="4"
                exercises={phases.coolDown} 
                onDrop={handleDrop} 
                onDragOver={handleDragOver}
                onDragStartMove={handleDragStartMove}
                onRemove={removeExercise}
...
// HELFER KOMPONENTE FÜR PHASEN (Damit der Code sauber bleibt)
function PhaseDropZone({ phaseKey, title, meta, color, number, exercises, onDrop, onDragOver, onRemove, isDragging, onDragStartMove }: any) {
    const colorClasses: any = {
        ...
        green: 'bg-emerald-500 shadow-emerald-100'
    };
            ...
            <div className="flex-1 space-y-3">
                ...
                {exercises.map((ex: any, idx: number) => (
                    <div 
                        ...
                        className="flex items-center gap-4 p-3 rounded-2xl bg-gray-50 border border-gray-100 group relative cursor-grab active:cursor-grabbing hover:shadow-sm transition-all"
                    >
                        <div className="w-10 h-10 rounded-xl bg-white overflow-hidden shrink-0 flex items-center justify-center">
