...
export default function NewTrainingSessionPage() {
  ...
  const [currentUserId, setCurrentUserId] = useState<string | null>(null); // <--- NEU: State für User ID
  ...
  // 1. Init: User & Übungen laden
  useEffect(() => {
    const initData = async () => {
      // A) User holen (WICHTIG für den Autor)
  ...
  const handleSave = async () => {
    ...
    if (!currentUserId) return alert("Fehler: Benutzer nicht identifiziert."); // Sicherheitscheck
    ...
    const { error } = await supabase.from('plans').insert({
        ...
        focus: '',
        content: phases,
        user_id: currentUserId // <--- WICHTIG: Hier wird der Autor gesetzt!
    });
