...
function PlanTable({ plans, onDuplicate, onDelete, isPast }: any) {
  return (
    <div className="w-full space-y-4">
      {/* Table Header */}
      <div className="grid grid-cols-[1.5fr_1fr_1.2fr_0.8fr_1.5fr_1.2fr] px-8 text-[9px] font-black uppercase tracking-widest text-gray-400 mb-2">
        ...
        <div className="text-left ml-4">Trainer</div>
        <div className="text-right px-4">Aktionen</div>
      </div>

      {/* Table Rows */}
      {plans.map((plan: any) => {
        // Extrahiere Initialen für den Trainer-Avatar (z.B. "Kevin Schneider" -> "KS")
        const trainerName = plan.author || "Kevin Schneider";
        const initials = trainerName.split(' ').map((n: string) => n[0]).join('').toUpperCase().substring(0, 2);

        return (
          <div key={plan.id} className={`bg-white rounded-[1.5rem] p-5 shadow-sm border border-white hover:border-gray-200 transition-all grid grid-cols-[1.5fr_1fr_1.2fr_0.8fr_1.5fr_1.2fr] items-center ${isPast ? 'opacity-75' : ''}`}>
            
            {/* 1. Datum & Name der Einheit */}
            <div>
              <div className="font-black text-sm">{new Date(plan.scheduled_at).toLocaleDateString('de-DE')}</div>
              <div className="text-[10px] font-bold text-gray-400 truncate uppercase tracking-tighter">{plan.title}</div>
            </div>

            {/* 2. Umfang */}
            <div className="flex justify-center">
              <span className="bg-[#F8F9FA] border border-gray-100 text-[9px] font-black px-3 py-1.5 rounded-lg uppercase text-gray-500 shadow-sm">
                {Object.values(plan.content || {}).flat().length} Übungen
              </span>
            </div>

            {/* 3. Erstellt am (Datum + Zeit) */}
            <div className="text-[10px] font-bold text-gray-400 text-center leading-tight">
              <div>{new Date(plan.created_at).toLocaleDateString('de-DE')}</div>
              <div className="text-[9px] opacity-70">{new Date(plan.created_at).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })} Uhr</div>
            </div>

            {/* 4. Uhrzeit des Trainings */}
            ...
            <div className="flex items-center gap-3 ml-4">
              <div className="w-8 h-8 bg-black rounded-full flex items-center justify-center text-[9px] font-bold text-white shrink-0 shadow-md">
                {initials}
              </div>
              <div className="text-[11px] font-black truncate">{trainerName}</div>
            </div>

            {/* 6. Aktionen */}
            <div className="flex justify-end gap-2 px-2">
              <ActionButton icon="ph-copy" onClick={() => onDuplicate(plan)} title="Duplizieren" />
              <ActionButton icon="ph-file-pdf" onClick={() => window.open(`/training/${plan.id}?print=true`, '_blank')} title="PDF Export" />
              <Link href={`/training/${plan.id}/edit`}>
                <ActionButton icon="ph-pencil-simple" title="Bearbeiten" />
              </Link>
              <ActionButton icon="ph-trash" onClick={() => onDelete(plan.id)} danger title="Löschen" />
            </div>
          </div>
        );
      })}
...
function ActionButton({ icon, onClick, danger, title }: any) {
  return (
    <button 
      onClick={onClick}
      title={title}
